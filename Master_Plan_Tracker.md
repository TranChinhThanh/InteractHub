...
- [/] Giai đoạn 5: Frontend Integration: 95% (đã có module auth frontend gần đầy đủ + Home Feed thin slice dùng React Query cache posts/infinite load-more + Create Post Form (RHF + upload/preview ảnh + mutation invalidate cache) + refactor `PostCard` với delete post và toggle like, đã fix ổn định 2 action delete/like qua e2e test; đã tích hợp CommentSection trong PostCard với list/create/delete comment và revalidate cache; đã hoàn thành Profile thin slice (view/edit + route + profile links); đã hoàn thành Notification thin slice (list/mark-as-read + route + navbar link); đã hoàn thành Search full-stack slice bằng debounce navbar + route `/search` + query param `q` + API `GET /users/search`; còn mở rộng UX/auth cases).
