...
- [/] **B2:** RESTful API + DTO (đã có Auth/Posts/Users/Friends/Stories + standardized response format cho Auth/Posts/Users/Friends/Stories và middleware 401/403; còn đủ bộ 6 controllers, >= 20 endpoints, mở rộng response format cho các controller mới, và Swagger examples request/response).
...
- [/] Hoàn thiện đủ bộ controller bắt buộc theo đề: `AuthController`, `PostsController`, `UsersController`, `FriendsController`, `StoriesController`, `NotificationsController` (đã có 5/6: Auth, Posts, Users, Friends, Stories).
...
- [/] Chuẩn hóa API response format thống nhất (`success`, `data`, `errors`) (đã áp dụng cho Auth + Posts + Users + Friends + Stories + middleware 401/403, còn mở rộng toàn bộ controller).
...
- [/] Giai đoạn 3: RESTful APIs nghiệp vụ chính: 71% (đã hoàn thành Posts Step 1-5 + chuẩn response format cho Auth/Posts/Users/Friends/Stories + dựng nền Generic Repository + Custom PostRepository + refactor `PostService` sang repository + Users Thin Slice 1-2 + Friends Thin Slice 1-2 (repository + DTO + service + DI + controller) + Stories Thin Slice 1-2 (repository + DTO + service + DI + controller); còn Notifications/... + 6 controllers/20 endpoints + mở rộng format toàn bộ).
