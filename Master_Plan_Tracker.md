...
- [/] Tạo Layouts cơ sở (Navbar, Sidebar, Main Content, Footer) + lazy loading/skeleton (đã có `MainLayout` 3 cột + `Navbar`, đã thêm `PageLoader` và bọc `Routes` bằng `Suspense` để lazy-load route pages; đã tích hợp `LeftSidebar` vào cột trái với điều hướng `NavLink` và trạng thái active/hover; đã tích hợp `RightSidebar` vào cột phải hiển thị trending hashtags).
